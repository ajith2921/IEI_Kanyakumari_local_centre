...
CREATE TABLE IF NOT EXISTS members (
    ...
    refresh_token_hash VARCHAR(255) DEFAULT '',
    refresh_token_expires_at TIMESTAMP NULL,
    membership_type VARCHAR(20) DEFAULT '',
    interest_area VARCHAR(180) DEFAULT '',
...
CREATE POLICY "Only authenticated users can delete contacts" ON contacts
    FOR DELETE USING (auth.role() = 'authenticated');
...
-- - members (for member profile images)
-- - gallery (for gallery images)
-- - activities (for activity images and PDFs)
-- - newsletters (for newsletter PDFs)
-- - downloads (for downloadable files)
-- ========================================

COMMENT ON TABLE users IS 'System admin users';
COMMENT ON TABLE members IS 'IEI Committee members';
COMMENT ON TABLE gallery IS 'Gallery images';
COMMENT ON TABLE newsletters IS 'Institution newsletters';
COMMENT ON TABLE activities IS 'Technical activities and events';
COMMENT ON TABLE facilities IS 'Institution facilities';
COMMENT ON TABLE downloads IS 'Downloadable resources';
COMMENT ON TABLE contacts IS 'Contact form submissions';
COMMENT ON TABLE conferences IS 'Conferences and events';
